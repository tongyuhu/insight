...
<section data-ng-controller="AddressController" data-ng-init="findOne()">
  <div class="secondary_navbar hidden-xs hidden-sm" scroll data-ng-class="{'hidden': !secondaryNavbar}" data-ng-show="address.addrStr" data-ng-init="hideSNavbar=0">
    <div class="container" data-ng-if="!hideSNavbar">
      ...
      <div class="col-md-4">
        <span class="txvalues txvalues-primary"><strong translate>Final Balance</strong> {{formatValue(address.balance)}}</span>
      </div>
    </div>
      ...
      <div class="col-md-10">
        <table class="table">
          ...
          <tr>
            ...
            <td class="ellipsis text-right">{{formatValue(address.totalReceived)}}</td>
          </tr>
          <tr>
            <td><strong translate>Total Sent</strong></td>
            <td class="ellipsis text-right">{{formatValue(address.totalSent)}}</td>
          </tr>
          <tr>
            <td><strong translate>Final Balance</strong></td>
            <td class="ellipsis text-right">{{formatValue(address.balance)}}</td>
          </tr>
          <tr>
